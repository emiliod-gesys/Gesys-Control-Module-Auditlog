...
# ---------------------------------------------------------------------------
# Guardia de re-entrada (thread-local)
# Evita que el código de tracking provoque más tracking al acceder al ORM
# (ej. lectura de valores anteriores dispara recompute → write → _patched_write)
# ---------------------------------------------------------------------------
_tracking_local = threading.local()


def _is_tracking():
    """True si estamos dentro de código de tracking en este hilo."""
    return getattr(_tracking_local, 'depth', 0) > 0


def _enter_tracking():
    _tracking_local.depth = getattr(_tracking_local, 'depth', 0) + 1


def _exit_tracking():
    _tracking_local.depth = max(0, getattr(_tracking_local, 'depth', 0) - 1)


def _cr_is_usable(cr):
    """
    Verifica que la conexión subyacente no esté en estado de error.
    Evita crear SAVEPOINTs en transacciones abortadas (InFailedSqlTransaction).
    """
    try:
        import psycopg2.extensions as pge
        conn = getattr(cr, '_cnx', None)
        if conn is None:
            return False
        status = conn.get_transaction_status()
        return status not in (
            pge.TRANSACTION_STATUS_INERROR,
            pge.TRANSACTION_STATUS_UNKNOWN,
        )
    except Exception:
        return True  # Si no se puede determinar, asumir OK
...
def _track_action_universal(self, action_type, description=None, action_method=None,
    ...
    """
    Método helper universal para registrar una acción en cualquier modelo.
    Nunca debe lanzar excepción ni afectar la transacción principal.
    """
    try:
        ...
        # Excluir modelos del sistema que no son útiles rastrear
        excluded_models = [
        ...
        # Reglas: excluir usuario segun config
        ...
        if not description:
            ...
            elif action_type in ('delete', 'unlink'):
                description = f"Eliminacion de {model_description}"
            elif action_type in ('validate', 'post'):
        ...
        _logger.debug(f"Error al rastrear acción universal en {getattr(self, '_name', 'unknown')}: {e}")


@api.model_create_multi
def _patched_create(self, vals_list):
    """Versión parcheada de create que agrega tracking universal."""
    ...
    # Saltar si ya estamos dentro de código de tracking (evita recursión)
    if _is_tracking() or self.env.context.get('gesys_control_skip_tracking'):
        return records

    if not _cr_is_usable(self.env.cr):
        return records
...
def _patched_write(self, vals):
    """Versión parcheada de write que agrega tracking universal."""
    if not vals:
        return _original_write(self, vals)

    # Saltar si ya estamos dentro de código de tracking (evita recursión)
    # Esto previene que la lectura de valores anteriores (que puede disparar
    # recompute → write) vuelva a entrar a este patch indefinidamente.
    if _is_tracking() or self.env.context.get('gesys_control_skip_tracking'):
        return _original_write(self, vals)
    ...
    # -----------------------------------------------------------------------
    # Fase pre-write: leer reglas, config y valores anteriores
    # Todo esto dentro del guard para evitar recursión si el acceso a campos
    # computados dispara otros writes/reads que vuelven a este patch.
    # -----------------------------------------------------------------------
    exclude_field_names = set()
    fast_mode = False
    old_values_by_id = {}

    if fields_to_track and _cr_is_usable(self.env.cr):
        _enter_tracking()
        try:
            # Regla: campos a excluir
            if 'gesys_control.rule' in self.env:
                try:
                    with self.env.cr.savepoint():
                        rule = self.env['gesys_control.rule'].sudo().search([
            ...
            # Modo rápido (sin captura de valores anteriores)
            ...
            # Capturar valores anteriores (solo si no es modo rápido)
            if fields_to_track and not fast_mode:
                for record in self:
                    old_values_by_id[record.id] = {
                        f: _serialize_value(record, f)
                        for f in fields_to_track if f in record._fields
                    }
        except Exception:
            pass
        finally:
    ...
    # -----------------------------------------------------------------------
    # Operación original — nunca se omite
    # -----------------------------------------------------------------------
    result = _original_write(self, vals)

    # -----------------------------------------------------------------------
    # Fase post-write: registrar cambios
    # -----------------------------------------------------------------------
    if result and fields_to_track and _cr_is_usable(self.env.cr) and not _is_tracking():
        _enter_tracking()
        try:
            for record in self:
                try:
                    if fast_mode:
                        ...
                        changes = {
                            f: {'old': old_vals.get(f), 'new': new_vals.get(f)}
                    ...
                    line_vals = [
                        {
                            'field_name': f,
                            'old_value_text': str(d.get('old') or ''),
...
def _patched_read(self, fields=None, load='_classic_read'):
    ...
    # Saltar si ya estamos en código de tracking o si el cursor no está OK
    if _is_tracking() or self.env.context.get('gesys_control_skip_tracking'):
        return result
    if not _cr_is_usable(self.env.cr):
        return result
...
def _patched_unlink(self):
    """Versión parcheada de unlink que agrega tracking universal."""
    # Saltar si ya estamos en código de tracking
    if _is_tracking() or self.env.context.get('gesys_control_skip_tracking'):
        return _original_unlink(self)

    if _cr_is_usable(self.env.cr):
        _enter_tracking()
        try:
            # Rastrear eliminación ANTES de eliminar (para tener el ID)
            for record in self:
                try:
...
def _patched_export_data(self, fields_to_export, *args, **kwargs):
    ...
    if not _is_tracking() and not self.env.context.get('gesys_control_skip_tracking'):
        try:
            _enter_tracking()
            try:
